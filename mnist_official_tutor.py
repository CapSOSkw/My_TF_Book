...
def cnn_model_fn(features, labels, mode):
    ...
    #  which specifies that this dimension should be dynamically computed.
    ...
    # conv1 outputs [-1, 28, 28, 32]

    pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size=[2, 2], strides=2, name='pool1')
    # pool outputs [-1, 14, 14, 32]
    ...
    # conv2 outputs [-1, 14, 14, 64]

    pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], strides=2, name='pool2')
    # pool2 outputs [-1, 7, 7, 64]

    pool2_flat = tf.reshape(pool2, [-1, 7*7*64], name='pool_flat')
...
def main(unused_argv):
    ...
    mnist_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir='/tmp/mnist_conv_model'
    )
